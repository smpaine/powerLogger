...
plot "< /usr/local/bin/sqlite3 datalog.db \"SELECT strftime('%m/%d/%Y',timestamp) AS 'Date', avgVoltage AS 'Average Voltage' FROM voltageLog\" 2>/dev/null | tail -n+3" using 1:2:xticlabels(1) title column(2)
